...
if ('History' in localStorage) {
    ...
    historyEl.textContent = savedHistory[i];
    }
}
searchHistory.addEventListener('click', historyButtonSearch)
function historyButtonSearch(event){
...
function speedySearch() {
    ...
    if (savedHistory && savedHistory.length > 0){
        savedHistory.push(boxValue)
        localStorage.setItem('History', JSON.stringify(savedHistory))
    } else {
        localStorage.setItem('History', JSON.stringify([boxValue]))
    }
